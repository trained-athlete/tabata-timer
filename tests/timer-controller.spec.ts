...
test.describe('computeSessionTotal', () => {
  ...
  test('EMOM-style totals', () => {
    const totals = { prep: 0, work: 60, rest: 0, rounds: 5, cycles: 1, longrest: 0 };
    expect(computeSessionTotal(totals)).toBe(300);
  });
});
...
test.describe('TimerController', () => {
  ...
  test('EMOM multiple rounds no rest', () => {
    const totals = { prep: 0, work: 1, rest: 0, rounds: 3, cycles: 1, longrest: 0 };
    const controller = new TimerController(totals);
    controller.start();
    controller._tick();
    let state = controller.getState();
